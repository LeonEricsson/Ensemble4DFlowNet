...
import shutil
import os
from .MetaCNNNet import MetaCNNNet
from . import utility, h5util, loss_utils

class MetaTrainerController:
    # constructor
    def __init__(self, patch_size, num_base_models, initial_learning_rate=1e-4, quicksave_enable=True, network_name='4DFlowNetMeta', resblock=8):
        """
        ...
        # Prepare Input 
        # u_1, ... , u_n, v_1, ... , v_n, w_1, ..., w_n
        input_layer = []
        for i in range(num_base_models):
            input_layer.append(tf.keras.layers.Input(shape=input_shape, name=f'u_{i+1}'))
            input_layer.append(tf.keras.layers.Input(shape=input_shape, name=f'v_{i+1}'))
            input_layer.append(tf.keras.layers.Input(shape=input_shape, name=f'w_{i+1}'))
        
        net = MetaCNNNet()
        self.predictions = net.build_network(input_layer, resblock)
    ...
    def loss_function(self, y_true, y_pred, mask, compartment):
        ...
        # Global batch weighting scheme (Inverse of Number of Samples) - counteract dataset imablance
        # table = tf.lookup.StaticHashTable(
        # initializer=tf.lookup.KeyValueTensorInitializer(list(weights.keys()), list(weights.values())),
        # default_value=-1.0)
        
        # compartment_weights = table.lookup(compartment)

        # Per batch weighting scheme (Inverse of Number of Samples) - counteract dataset imbalance.
        _, idx, counts = tf.unique_with_counts(compartment)
        num_compartments = tf.shape(counts)[0]
        compartment_weights = 1.0 / tf.cast(counts, tf.float32) # weighting
        compartment_weights = compartment_weights / tf.reduce_sum(compartment_weights) * tf.cast(num_compartments, dtype=tf.float32) # Normalize
        compartment_weights = tf.gather(compartment_weights, idx) # map weights to each sample
                
        total_loss = total_loss * compartment_weights
    ...
    def _prepare_logfile_and_summary(self):
        ...
        # Temporary solution, original one is above
        directory = "."
        fname = "meta_trainer.py"
        dest_fpath = os.path.join(self.model_dir,"backup_source",directory, fname)
        os.makedirs(os.path.dirname(dest_fpath), exist_ok=True)
    ...
    def train_step(self, data_pairs):
        uvw_m, u_hr,v_hr, w_hr, _, mask, compartment = data_pairs
        input_data = tf.unstack(uvw_m, axis=1)
        hires = tf.squeeze(tf.stack([u_hr, v_hr, w_hr], axis=-1), axis=1)
        with tf.GradientTape() as tape:
            # training=True is only needed if there are layers with different
            # behavior during training versus inference (e.g. Dropout).
            predictions = self.model(input_data, training=True)
            loss = self.calculate_and_update_metrics(hires, predictions, mask, compartment, 'train')
    ...
    def test_step(self, data_pairs):
        ...
        self.calculate_and_update_metrics(hires, predictions, mask, compartment, 'val')
       

        return predictions

    def calculate_and_update_metrics(self, hires, predictions, mask, compartment, metric_set):
        loss, mse, divloss = self.loss_function(hires, predictions, mask, compartment)
    ...
    # Stable until epoch 60, then smooth decay
    def decay2(self, epoch):
        if epoch < 60:
            return self.learning_rate
        if epoch >= 60:
            return self.learning_rate/(1 + 0.1*(epoch - 30))
    
    # Decay with sqrt(2) every 25 epochs    
    def decay3(self, epoch):
        if epoch != 0 and epoch % 25 == 0:
            self.learning_rate = self.learning_rate * 1/np.sqrt(2)
        return self.learning_rate
    ...
    def train_network(self, trainset, valset, n_epoch, testset=None):
        ...
        #tf.profiler.experimental.start(f"{self.model_dir}/tensorboard/profiler")
        ...
        for epoch in range(n_epoch):
            ...
            # Learning rate decay
            self.optimizer.learning_rate = self.decay3(epoch)
    ...
    def _update_summary_logging(self, epoch):
        ...
        with self.train_writer.as_default():
            tf.summary.scalar(f"{self.network_name}/§", self.optimizer.lr, step=epoch)
    ...
    def quicksave(self, testset, epoch_nr):
        ...
        for i, (data_pairs) in enumerate(testset):
            u,v,w, u_mag, v_mag, w_mag, u_hr,v_hr, w_hr, venc, mask, compartment = data_pairs
            hires = tf.concat((u_hr, v_hr, w_hr), axis=-1)
            input_data = [u,v,w, u_mag, v_mag, w_mag]

            preds = self.model.predict(input_data)

            loss_val, mse, divloss = self.loss_function(hires, preds, mask, compartment)
