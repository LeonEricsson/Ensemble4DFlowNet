...
class PatchHandler3D():
    ...
    def initialize_dataset(self, indexes, shuffle, n_parallel=None, drop_remainder=False):
        '''
            Input pipeline.
    ...
    def load_patches_from_index_file(self, indexes):
        ...
        compartment = bytes.decode(indexes[10].numpy())
        ...
        # If we have dynamic mask, use it REMEMBER TO CHANGE!!
        with h5py.File(hd5path, 'r') as hl:
            mask = tf.convert_to_tensor(hl.get(self.mask_colname))
