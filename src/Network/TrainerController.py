...
class TrainerController:
    # constructor
    def __init__(self, patch_size, res_increase, initial_learning_rate, quicksave_enable, network_name, nr_low_block, nr_hi_block, 
                 type_low_resblock, type_hi_resblock, central_upsampling):
        """
        ...
        net = SR4DFlowNet(res_increase, central_upsampling, type_hi_resblock, type_low_resblock)
        self.predictions = net.build_network(u, v, w, u_mag, v_mag, w_mag, nr_low_block, nr_hi_block)
    ...
    def loss_function(self, y_true, y_pred, mask, compartment):
        ...
        # boundary mask to weight differently
        ...
        core_mse = mse * core_mask * core_w
        boundary_mse = mse * boundary_mask * boundary_w

        sum_core_mse = tf.reduce_sum(core_mse, axis=[1,2,3])
        sum_boundary_mse = tf.reduce_sum(boundary_mse, axis=[1,2,3])

        #fluid_mse = mse * mask
        mask_size = tf.reduce_sum(mask, axis=[1,2,3])

        fluid_mse = (sum_core_mse + sum_boundary_mse) / (mask_size + epsilon)


        non_fluid_mse = mse * non_fluid_mask * non_fluid_w
        non_fluid_mask_size = tf.reduce_sum(non_fluid_mask, axis=[1,2,3])

        non_fluid_mse = tf.reduce_sum(non_fluid_mse, axis=[1,2,3]) / (non_fluid_mask_size + epsilon)
    ...
    def _prepare_logfile_and_summary(self):
        ...
        self.variable_file = self.model_dir + '/variables.json'
        ...
        utility.log_to_file(self.logfile, 'Distribution:\n')
        ...
        # Temporary solution to save only trainer.py, original one is above
        directory = "."
        fname = "base_trainer.py"
        dest_fpath = os.path.join(self.model_dir,"backup_source",directory, fname)
        os.makedirs(os.path.dirname(dest_fpath), exist_ok=True)
    ...
    def save_best_model(self):
        """
            Save model weights and also optimizer weights to enable restore model
