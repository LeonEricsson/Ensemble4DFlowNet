...
def generate_random_patches(input_filename, target_filename, output_filename, index, n_patch, binary_mask, patch_size, minimum_coverage, empty_patch_allowed, label, apply_all_rotation=True):
    ...
    while j < n_patch:
        ...
        patch = PatchData(input_filename, target_filename, patch_size, label)
...
class PatchData:
    def __init__(self, source_file, target_file, patch_size, label):
    ...
    def write_to_csv(self, output_filename):
        ...
        with open(output_filename, mode='a', newline='') as csv_file:
            ...
            'coverage': self.coverage, 'compartment': self.label})