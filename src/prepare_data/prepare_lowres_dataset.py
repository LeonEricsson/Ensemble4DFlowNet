...
# Calculate the padding for each of the 3 image dimensions
def downsample_check(mask, downsample):
    downsample_shape = np.ceil((np.asarray(mask.shape) / 2))
    non_float = np.ceil(downsample_shape / 2)
    upsampled_shape = non_float * 2 * downsample
    padding = (upsampled_shape - np.asarray(mask.shape)).astype(np.int32)
       
    return padding

# Pad the hard coded columns in the HDF5 file.
def pad(file, padding):
    col_names = ['u', 'v', 'w', 'mag_u', 'mag_v', 'mag_w', 'mask']
    with h5py.File(file, mode = 'a') as hf:
        # Iterate over all datasets to change
        print("padding", padding)
        for ds_name in hf.keys():
            if ds_name in col_names:
                ds = hf[ds_name]
                ds_dtype = ds.dtype
        
                # Pad each dimension equally on each side.
                if len(ds.shape) == 4:
                    padded_ds = np.pad(ds, ((0,0), (padding[0]//2, padding[0]//2), (padding[1]//2, padding[1]//2), (padding[2]//2, padding[2]//2)), mode='constant')
                elif len(ds.shape) == 3:
                    padded_ds = np.pad(ds, ((padding[0]//2, padding[0]//2), (padding[1]//2, padding[1]//2), (padding[2]//2, padding[2]//2)), mode='constant')
                else: 
                    print("Unknown dataset dimension: {ds.shape}")
                    
                del hf[ds_name]
                new_ds = hf.create_dataset(ds_name, padded_ds.shape, dtype=ds_dtype)
                new_ds[:, :, :] = padded_ds
...
if __name__ == '__main__':
    # Config
    base_path = '../../data'
    # Put your path to Hi-res Dataset
    ...
    # If PAD is true the HR data will be modified when necessary
    PAD = True
    ...
    venc_values =  np.asarray([0.3, 0.6, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4, 4.5, 5]) # in m/s - note: added 4, 4.5 & 5 for cardiac data
    ...
    with h5py.File(input_filepath, mode = 'r' ) as hf:
        ...
        if len(mask.shape) == 4: 
            mask = mask[0]
        # Determine if padding is necessary before downsampling
        padding = downsample_check(mask, downsample)
        data_count = len(hf.get("u"))
    
    # Pre-process the HR data by padding it to ensure dimensional continuity post downsample.
    if padding.any() and PAD:
        print("Necessary pre-processing of HR data")
        pad(input_filepath, padding)
    ...
    for idx in range(data_count):
        ...
        with h5py.File(input_filepath, mode = 'r' ) as hf:
            ...
            u_max = 'u_max' if 'u_max' in hf.keys() else 'max_u'
            v_max = 'v_max' if 'v_max' in hf.keys() else 'max_v'
            w_max = 'w_max' if 'w_max' in hf.keys() else 'max_w'
